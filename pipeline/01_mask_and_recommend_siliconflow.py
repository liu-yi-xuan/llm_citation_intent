#%%
"""
Step 01 (SiliconFlow) — Masking & LLM citation reconstruction
=============================================================
...
Provider: SiliconFlow (international, https://api.siliconflow.com/v1).
Keys:     siliconflow_keys.json or env SILICONFLOW_API_KEYS / SILICONFLOW_API_KEY.
...
Progress monitoring:
  ...
  - Log file at logs/03_sf_run_{model_slug}.log
      tail -f "logs/03_sf_run_deepseek-ai_DeepSeek-V3.2.log"
  - Live snapshot at logs/03_sf_progress_{model_slug}.json
      cat "logs/03_sf_progress_deepseek-ai_DeepSeek-V3.2.json"

Usage:
    python "03 llm_cite_recommendation_sf.py"
...
# SiliconFlow model slugs. Override with env SILICONFLOW_MODELS (comma-separated).
...
def parse_models() -> list[str]:
    """
    Comma-separated SiliconFlow model slugs from env SILICONFLOW_MODELS.
    """
    raw = os.getenv("SILICONFLOW_MODELS", DEFAULT_MODELS)
...
def _load_api_keys() -> list:
    ...
    Priority:
      1) KEYS_FILE JSON: {"siliconflow_api_keys": ["...", "..."]}
      2) SILICONFLOW_API_KEYS (comma-separated)
    ...
    if os.path.exists(KEYS_FILE):
        try:
            with open(KEYS_FILE, "r", encoding="utf-8") as f:
                payload = json.load(f)
            file_keys = (
                payload.get("siliconflow_api_keys")
    ...
    keys_csv = os.getenv("SILICONFLOW_API_KEYS", "").strip()
    if keys_csv:
        return [k.strip() for k in keys_csv.split(",") if k.strip()]

    single_key = os.getenv("SILICONFLOW_API_KEY", "").strip()
    if single_key:
        return [single_key]
...
CLIENTS = [
    OpenAI(api_key=key, base_url=SILICONFLOW_BASE_URL)
    for key in API_KEYS
]
...
def call_llm(
    ...
    try:
        ...
        response, used_json_mode = create_chat_completion(
            ...
            max_tokens=min(2000, 200 + 130 * required_num_citations),
        )
...
def call_llm_with_retry(
    ...
    for attempt in range(MAX_RETRIES):
        ...
        #  - api_error / validation_error -> retry with backoff
        #  - json_parse_error -> almost always max_tokens truncation; retrying
        #    will hit the same cap, so return immediately and save the error.
...
def fix_errored_contexts_in_dir(output_dir: str, model: str, run_logger=None) -> dict:
    ...
    # Pre-scan to count work and skip clean papers.
    work = []  # list of (response_path, [errored_context_indices])
    initial_errors = 0
    for fp in files:
        try:
            with open(fp) as f:
    ...
    for fp, idxs in pbar:
        ...
        pbar.set_postfix(
            fixed=summary["contexts_fixed"],
            still=summary["contexts_still_error"],
        )
        if run_logger:
            run_logger.info(
...
if __name__ == "__main__":
    ...
    if not CLIENTS:
        raise RuntimeError(
            "No API keys found. Set siliconflow_keys.json, SILICONFLOW_API_KEYS, "
    ...
    print("=" * 70)
    print("LLM Citation Recommendation (Sycophancy Experiment) — SiliconFlow")
    print(f"Models: {models}")
    print(f"Provider base URL: {SILICONFLOW_BASE_URL}")
    ...
    print(f"Logs:        {LOG_DIR}/{LOG_TAG}_run_{{model_slug}}.log")
    print(f"Live state:  {LOG_DIR}/{LOG_TAG}_progress_{{model_slug}}.json")
    print("=" * 70)
    ...
    for model in models:
        ...
        run_logger.info(f"Provider: {SILICONFLOW_BASE_URL}")
        ...
        # Phase 1: fix-up errored contexts in already-saved files (in place).
        ...
        # Cost estimate (rough; SiliconFlow DeepSeek-V3.2 pricing varies).
        ...
        write_progress_snapshot(model_dir, {
            "model": model,
            "provider_base_url": SILICONFLOW_BASE_URL,
